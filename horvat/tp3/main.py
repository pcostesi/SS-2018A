...
def generate(N, L, r, v, m, r2, m2):
    ...
    particles = [Particle(x = L/2, y = L/2, vx = 0, vy = 0, r = r2, m = m2, color = "0")]
...
def save_file(file_string, t = 0, N = 0, v=0):
    with open('output/brownian' + (str(N) if N > 0 else 'e') + ' t'  + str(t)+ ' v'  + str(v)+ '.txt', 'w') as outfile:
        outfile.write(file_string)

def save_data(file_string, N = 0, v=0):
    with open('output/data' + (str(N) if N > 0 else 'e') +' v'+ str(v)+ '.txt', 'w') as outfile:
        outfile.write(file_string)
...
def start(L, particles, dt, v):
    ...
    while not ( (big_particle.r - big_particle.x) == x1 or
        ...
        if i % 100 == 0:
            ...
            print big_particle
            if i % 1000 == 0:
                save_file(file_string = file_string, t=t_sum, N = len(particles) -1, v=v)
    ...
    save_file(file_string = file_string, N = len(particles) -1 , t = t_sum, v=v)
    ...
    data_str += "i =" + str(i) + '\n'
    save_data(data_str, N = len(particles) -1,v=v)
    ...
    ax.set_xlim(-0.0,0.5)
    ax.set_ylim(-0.0,0.5)
    plt.savefig('output/distance-' + str(v) + '.png')
...
def main():
    ...
    if N is int:
        ...
        for n in N:
            for vv in [v*0.5,v,v*2]:
                particles = data['particles'] if 'particles' in data else generate(N = n, L = L, r = r, v = vv, m = m, r2 = r2, m2 = m2)
                start(L = L, particles = particles, dt = dt, v = vv)
